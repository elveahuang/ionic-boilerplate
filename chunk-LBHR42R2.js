import {
  CommonModule,
  CoreModule,
  IonContent,
  IonHeader,
  IonTitle,
  IonToolbar,
  ɵsetClassDebugInfo,
  ɵɵStandaloneFeature,
  ɵɵdefineComponent,
  ɵɵelementEnd,
  ɵɵelementStart,
  ɵɵtext
} from "./chunk-A4W7M7UA.js";
import "./chunk-ZMTS6FB7.js";
import "./chunk-IA6T7H56.js";
import "./chunk-LYWIERVT.js";
import "./chunk-6AZWWDGR.js";
import "./chunk-K7VTRZGW.js";
import "./chunk-U4D2FFXQ.js";
import "./chunk-EEMSZQXW.js";
import "./chunk-QBNF63C4.js";
import "./chunk-25YLPDTL.js";
import "./chunk-TXKIG2XS.js";
import "./chunk-PB3C3JLH.js";
import "./chunk-NPVKIRVG.js";
import "./chunk-26SQRFQG.js";
import "./chunk-X75KCS73.js";
import "./chunk-HSGDXAPU.js";
import "./chunk-DKWROOMG.js";
import "./chunk-NU6NIPSH.js";
import "./chunk-DTZLP7YH.js";
import "./chunk-K6JFMU26.js";
import "./chunk-IVNTVPZ7.js";
import "./chunk-E6YUNWHO.js";
import "./chunk-ALHM76N4.js";
import "./chunk-NS6ZA2PN.js";
import "./chunk-GT7UU2G5.js";
import "./chunk-LFTLW3GW.js";
import "./chunk-OJBVTQPQ.js";
import "./chunk-V5WFMJ5J.js";
import "./chunk-C7BVS75L.js";
import "./chunk-LVAVXFQB.js";
import "./chunk-ZM7N4A2I.js";
import "./chunk-A6W4RR5W.js";
import "./chunk-QS2CILT5.js";
import "./chunk-IBIAN5LZ.js";
import "./chunk-R6AEDFCP.js";

// src/app/core/pages/about/about.page.ts
var _AboutPage = class _AboutPage {
};
_AboutPage.\u0275fac = function AboutPage_Factory(t) {
  return new (t || _AboutPage)();
};
_AboutPage.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _AboutPage, selectors: [["app-about"]], standalone: true, features: [\u0275\u0275StandaloneFeature], decls: 6, vars: 0, template: function AboutPage_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "ion-header")(1, "ion-toolbar")(2, "ion-title");
    \u0275\u0275text(3, "About");
    \u0275\u0275elementEnd()()();
    \u0275\u0275elementStart(4, "ion-content");
    \u0275\u0275text(5, "About");
    \u0275\u0275elementEnd();
  }
}, dependencies: [CommonModule, CoreModule, IonContent, IonHeader, IonTitle, IonToolbar] });
var AboutPage = _AboutPage;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AboutPage, { className: "AboutPage", filePath: "src/app/core/pages/about/about.page.ts", lineNumber: 13 });
})();
export {
  AboutPage
};
